...
{
    token = strtok(token, " ");
    token = strtok(NULL, " ");  //此时的token为要切换的路径
    if(token){
        if(_chdir(token) < 0){  //切换工作路径失败
            memset(sendbuf, 0, 100); //清零防止出现乱码
            strcpy_s(sendbuf, "Directory doesn't exist");
            sendlen = send(socket_receive, sendbuf, 100, 0);
        }
        else{
            memset(sendbuf, 0, 100); //清零防止出现乱码
            strcpy_s(sendbuf, "Directory successfully changed");
            sendlen = send(socket_receive, sendbuf, 100, 0);
        }
    }
    else{   //用户输入的cd后面没有参数
        send_Msg(clifd, "请输入路径");
    }
}
